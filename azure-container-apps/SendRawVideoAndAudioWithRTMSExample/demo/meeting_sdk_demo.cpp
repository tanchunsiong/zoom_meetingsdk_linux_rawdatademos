...
ZoomSDKVirtualAudioMicEvent* audio_source;
GMainLoop* loop;
...
void CheckAndStartRawSending(bool isVideo, bool isAudio) {
	...
	if (isVideo) {
	   ...
	   printf(" p_videoSourceHelper->setExternalVideoSource(virtual_camera_video_source);");
...
void turnOnSendVideoAndAudio() {
	printf("turnOnSendVideoAndAudio now...\n");
	//testing WIP
	if (SendVideoRawData) {
		IMeetingVideoController* meetingVidController = m_pMeetingService->GetMeetingVideoController();
...
int main(int argc, char* argv[])
{
  ...
  std::thread([full_ws_url]() {
    connect_to_event_server(full_ws_url);
}).detach();

	virtual_camera_video_source = new ZoomSDKVideoSource();
	audio_source = new ZoomSDKVirtualAudioMicEvent();
	set_video_source_reference(virtual_camera_video_source);
	set_audio_source_reference(audio_source);
