...
MeetingChatEventListener::MeetingChatEventListener(void(*turnOnSendShareVideoAndAudio)(), void(*turnOffSendShareVideoAndAudio)()) {


	turnOnSendShareVideoAndAudio_ = turnOnSendShareVideoAndAudio;
	turnOffSendShareVideoAndAudio_ = turnOffSendShareVideoAndAudio;
	onChatMessageReceived_ = nullptr;
}

MeetingChatEventListener::MeetingChatEventListener(void(*onChatMessageReceived)(IChatMsgInfo*)) {
	turnOnSendShareVideoAndAudio_ = nullptr;
	turnOffSendShareVideoAndAudio_ = nullptr;
...
void MeetingChatEventListener::onChatMsgNotification(IChatMsgInfo* chatMsg, const zchar_t* content)
{
	if (onChatMessageReceived_) {
		...
		turnOnSendShareVideoAndAudio_();
	}
	else if (ZCharStringMatches(chatMsg->GetContent(), "turnOff")) {
		turnOffSendShareVideoAndAudio_();
	}
