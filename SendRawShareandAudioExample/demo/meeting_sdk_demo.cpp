...
// External share video and share audio sources.
...
// Source files copied to demo/bin by CMake.
...
bool SendShareVideoRawData = false;
bool SendShareAudioRawData = false;
bool chatDemo = true;

ZoomSDKShareSource* share_video_source = nullptr;
ZoomSDKShareAudioSource* share_audio_source = nullptr;
...
void CheckAndStartRawShareSending(bool send_video, bool send_audio) {
	if (!send_video && !send_audio) {
		return;
	}
	if (!send_video) {
		printf("Raw share mode requires SendShareVideoRawData\n");
		return;
	}

	IZoomSDKShareSourceHelper* helper = GetRawdataShareSourceHelper();
	if (!helper) {
		printf("Failed to get raw-data share source helper\n");
		return;
	}

	if (send_video && !share_video_source) {
		share_video_source = new ZoomSDKShareSource(DEFAULT_VIDEO_SOURCE);
	}
	if (send_audio && !share_audio_source) {
		share_audio_source = new ZoomSDKShareAudioSource(DEFAULT_AUDIO_SOURCE);
	}

	SDKError error = helper->setExternalShareSource(
		share_video_source,
		send_audio ? share_audio_source : nullptr);

	if (error == SDKERR_SUCCESS) {
		printf(
			"%s started\n",
			send_audio
				? "Raw share video and audio sources"
				: "Raw share video source");
	} else {
		printf("Failed to start raw share source, error code: %d\n", error);
	}
}
...
void turnOnSendShareVideoAndAudio() {
	CheckAndStartRawShareSending(SendShareVideoRawData, SendShareAudioRawData);
}
void turnOffSendShareVideoAndAudio() {
	IMeetingShareController* share_controller = m_pMeetingService->GetMeetingShareController();
	if (share_controller) {
		SDKError error = share_controller->StopShare();
		printf("StopShare result: %d\n", error);
	}
}
...
void onInMeeting() {
	...
	if (chatDemo) {
		...
		meetingchatcontroller->SetEvent(new MeetingChatEventListener(&turnOnSendShareVideoAndAudio, &turnOffSendShareVideoAndAudio));
