				...
				if (isVideo) {
					...
					for (int i = 0; i < pList->GetCount(); ++i) {
						...
						if (p != getMyself()) {
							...
							std::cout << "Skipping self User ID : " << userID << std::endl;
						}
					}
...
void listenForPythonMessages(void (*sendMessage)(const std::string&)) {
	...
	listen_addr.sin_port = htons(9090);  // Listening on port 9090
	listen_addr.sin_addr.s_addr = INADDR_ANY;

	// Bind the socket
	if (bind(listen_socket, (struct sockaddr*)&listen_addr, sizeof(listen_addr)) < 0) {
		std::cerr << "Failed to bind listening socket: " << strerror(errno) << std::endl;
		close(listen_socket);
		return;
	}
...
void sendMessage(const std::string& message) {
	...
	//print err
	if (err != SDKERR_SUCCESS) {
		std::cout << "Error sending chat message: " << err << std::endl;
	}
	else {
