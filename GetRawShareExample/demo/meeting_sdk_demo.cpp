...
//references for GetShareRawData
#include "rawdata/rawdata_renderer_interface.h"
#include "rawdata/zoom_rawdata_api.h"

//needed for raw recording permission
...
std::map<unsigned int, IZoomSDKRenderer*> shareRenderers;
std::map<unsigned int, ZoomSDKRawDataPipeDelegate*> shareDelegates;

//this is used to get a userID, there is no specific proper logic here. It just gets the first userID.
//userID is needed to resolve the share source owner.
unsigned int userID;
...
//this will enable or disable logic to get raw share/screen data
//do note that this will be overwritten by config.json
bool GetShareRawData = true;
...
bool IsShareBeginStatus(SharingStatus status) {
	return status == Sharing_Self_Send_Begin ||
		status == Sharing_Other_Share_Begin ||
		status == Sharing_View_Other_Sharing ||
		status == Sharing_Resume;
}

bool IsShareEndStatus(SharingStatus status) {
	return status == Sharing_Self_Send_End ||
		status == Sharing_Other_Share_End ||
		status == Sharing_Pause;
}

void StopShareRawData(unsigned int shareSourceID) {
	if (shareSourceID == 0) {
		std::cout << "[GetRawShare] stop skipped: shareSourceID is 0" << std::endl;
		return;
	}

	auto rendererIter = shareRenderers.find(shareSourceID);
	if (rendererIter == shareRenderers.end()) {
		std::cout << "[GetRawShare] stop ignored: no active renderer for shareSourceID=" << shareSourceID << std::endl;
		return;
	}

	std::cout << "[GetRawShare] stopping raw share subscription: shareSourceID=" << shareSourceID << std::endl;
	rendererIter->second->unSubscribe();
	destroyRenderer(rendererIter->second);
	shareRenderers.erase(rendererIter);

	auto delegateIter = shareDelegates.find(shareSourceID);
	if (delegateIter != shareDelegates.end()) {
		delete delegateIter->second;
		shareDelegates.erase(delegateIter);
	}
}

void SubscribeToShareRawData(ZoomSDKSharingSourceInfo shareInfo, const char* reason) {
	if (shareInfo.shareSourceID == 0) {
		std::cout << "[GetRawShare] subscribe skipped: shareSourceID is 0, userid=" << shareInfo.userid << " reason=" << reason << std::endl;
		return;
	}

	if (shareRenderers.find(shareInfo.shareSourceID) != shareRenderers.end()) {
		std::cout << "[GetRawShare] subscribe ignored: already subscribed to shareSourceID=" << shareInfo.shareSourceID << " reason=" << reason << std::endl;
		return;
	}

	m_pParticipantsController = m_pMeetingService->GetMeetingParticipantsController();
	IUserInfo* p = m_pParticipantsController->GetUserByUserID(shareInfo.userid);
	if (!p) {
		std::cout << "[GetRawShare] subscribe skipped: cannot resolve userid=" << shareInfo.userid << " shareSourceID=" << shareInfo.shareSourceID << std::endl;
		return;
	}

	std::cout
		<< "[GetRawShare] subscribing after share status "
		<< shareInfo.status
		<< ": userid=" << shareInfo.userid
		<< " userName=" << p->GetUserName()
		<< " shareSourceID=" << shareInfo.shareSourceID
		<< " reason=" << reason
		<< std::endl;

	IZoomSDKRenderer* renderer = nullptr;
	ZoomSDKRawDataPipeDelegate* rawdatapipedelegate = new ZoomSDKRawDataPipeDelegate();
	SDKError createErr = createRenderer(&renderer, rawdatapipedelegate);
	std::cout << "[GetRawShare] createRenderer result: shareSourceID=" << shareInfo.shareSourceID << " sdkError=" << createErr << std::endl;
	if (createErr != SDKERR_SUCCESS || !renderer) {
		delete rawdatapipedelegate;
		return;
	}

	SDKError subscribeErr = rawdatapipedelegate->SubscribeShareSource(p, shareInfo.shareSourceID, renderer);
	if (subscribeErr != SDKERR_SUCCESS) {
	...
	shareRenderers[shareInfo.shareSourceID] = renderer;
	shareDelegates[shareInfo.shareSourceID] = rawdatapipedelegate;
}

void HandleShareSourceChanged(ZoomSDKSharingSourceInfo shareInfo) {
	if (IsShareBeginStatus(shareInfo.status)) {
		SubscribeToShareRawData(shareInfo, "status-started");
		return;
	}

	if (IsShareEndStatus(shareInfo.status)) {
		StopShareRawData(shareInfo.shareSourceID);
		return;
	}

	std::cout
		<< "[GetRawShare] share status observed without subscription change: userid=" << shareInfo.userid
		<< " shareSourceID=" << shareInfo.shareSourceID
		<< " status=" << ShareStatusName(shareInfo.status) << "(" << shareInfo.status << ")"
		<< std::endl;
}

void SubscribeToActiveShareSources() {
	...
	for (int i = 0; i < pList->GetCount(); ++i) {
		...
		IList<ZoomSDKSharingSourceInfo>* shareList = meetingShareController->GetSharingSourceInfoList(userID);
		if (!shareList) {
			continue;
		}

		std::cout << "[GetRawShare] existing share sources for userID=" << userID << ": " << shareList->GetCount() << std::endl;
		for (int j = 0; j < shareList->GetCount(); ++j) {
			ZoomSDKSharingSourceInfo shareInfo = shareList->GetItem(j);
			std::cout
				<< "[GetRawShare] existing share source: userid=" << shareInfo.userid
				<< " shareSourceID=" << shareInfo.shareSourceID
				<< " status=" << ShareStatusName(shareInfo.status) << "(" << shareInfo.status << ")"
				<< " contentType=" << shareInfo.contentType
				<< std::endl;
			if (IsShareBeginStatus(shareInfo.status)) {
				SubscribeToShareRawData(shareInfo, "existing-active-share");
			}
		}
	}
}
...
void CheckAndStartRawRecording(bool isShare) {
	...
	if (isShare) {
		...
		if (err2 == SDKERR_SUCCESS) {
			...
			if (err1 != SDKERR_SUCCESS) {
				...
				std::cout << "[GetRawShare] raw recording started; waiting for share begin status before subscribing" << std::endl;
				SubscribeToActiveShareSources();
			}
		}
		else {
...
void onIsHost() {
	printf("Is host now...\n");
	CheckAndStartRawRecording(GetShareRawData);
...
void ReadJsonSettings()
{
	...
	if (!config_json.is_null()) {
		...
		processJsonBoolean(config_json, "GetShareRawData", GetShareRawData);
...
void CleanSDK()
{
	...
	for (auto& item : shareRenderers) {
		item.second->unSubscribe();
		destroyRenderer(item.second);
	}
	shareRenderers.clear();
	for (auto& item : shareDelegates) {
		delete item.second;
	}
	shareDelegates.clear();
...
void JoinMeeting()
{
	...
	IMeetingShareController* meetingShareController = m_pMeetingService->GetMeetingShareController();
	SDKError shareEventError = meetingShareController->SetEvent(new MeetingShareCtrlEventListener(&HandleShareSourceChanged));
