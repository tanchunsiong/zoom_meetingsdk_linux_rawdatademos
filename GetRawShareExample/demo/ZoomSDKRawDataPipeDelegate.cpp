...
SDKError ZoomSDKRawDataPipeDelegate::SubscribeShareSource(IUserInfo* user, uint32_t shareSourceID, IZoomSDKRenderer* renderer)

{
	instance_id_ = instance_count++;
	user_ = user;
	subscribed_share_source_id_ = shareSourceID;
	printf(
		"[GetRawShare] [%d] subscribing: userId=%u userName=%s shareSourceID=%u rawType=RAW_DATA_TYPE_SHARE\n",
		instance_id_,
		user ? user->GetUserID() : 0,
		user ? user->GetUserName() : "(null)",
		shareSourceID
	);
	SDKError err = renderer->subscribe(shareSourceID, RAW_DATA_TYPE_SHARE);
	printf(
		"[GetRawShare] [%d] subscribe result: shareSourceID=%u sdkError=%d\n",
		instance_id_,
		shareSourceID,
		err
	);
...
{
	printf("[GetRawShare] [%d] renderer destroyed: shareSourceID=%u\n", instance_id_, subscribed_share_source_id_);
	fflush(stdout);
}
...
void ZoomSDKRawDataPipeDelegate::onRawDataFrameReceived(YUVRawDataI420* data)
{
	...
	raw_frame_count++;
	if (raw_frame_count == 1 || raw_frame_count % 100 == 0)
	{
		printf(
			"[GetRawShare] [%d] raw share frame received: frame=%d userId=%u shareSourceID=%u sourceID=%d size=%dx%d bufferLen=%d rotation=%d\n",
			instance_id_,
			raw_frame_count,
			userID,
			subscribed_share_source_id_,
			sourceID,
			width,
			height,
			bufLen,
...
{
	const char* statusText = status == RawData_On ? "RawData_On" : "RawData_Off";
	printf(
		"[GetRawShare] [%d] raw data status changed: userId=%u shareSourceID=%u status=%s(%d)\n",
		instance_id_,
		user_ ? user_->GetUserID() : 0,
		subscribed_share_source_id_,
		statusText,
		status
	);
...
int ZoomSDKRawDataPipeDelegate::ffmpeg_start(const char* userName, uint userID, int sourceID)
{
	...
	printf("[GetRawShare] [%d] opening output file: %s\n", instance_id_, fn_out);
	fflush(stdout);
