...
const envHelp = {
  ...
  ECS_TASK_CPU: 'Fargate task CPU units. 256 means 0.25 vCPU.',
  ECS_TASK_MEMORY: 'Fargate task memory in MiB. 512 means 0.5GB RAM.',
  ECS_SUBNETS: 'Comma-separated subnet IDs where Fargate runner tasks launch.',
  ECS_SECURITY_GROUPS: 'Comma-separated security group IDs for Fargate runner tasks.',
  ECS_ASSIGN_PUBLIC_IP: 'Set true for public subnet outbound internet without NAT, or false for private subnet through NAT.',
...
function renderStatus(status) {
  statusEl.innerHTML = [
    ...
    envPill('ECS Cluster', Boolean(status.ecs?.cluster), status.ecs?.cluster || ''),
    envPill('ECS Task Definition', Boolean(status.ecs?.taskDefinition), status.ecs?.taskDefinition || ''),
    envPill('ECS Subnets', Boolean(status.ecs?.hasSubnets)),
    envPill('ECS Security Groups', Boolean(status.ecs?.hasSecurityGroups)),
    `<div class="pill"><span>Fargate CPU / RAM</span><strong>${escapeHtml(status.ecs?.taskCpu || '')} / ${escapeHtml(status.ecs?.taskMemory || '')}</strong><small>256 / 512 = 0.25 vCPU / 0.5GB</small></div>`,
    `<div class="pill"><span>ECS Region</span><strong>${escapeHtml(status.ecs?.region || '')}</strong></div>`,
    `<div class="pill"><span>ECS Project</span><strong>${escapeHtml(status.ecs?.project || '')}</strong></div>`
  ].join('');
...
function renderContainers(containers) {
  if (!Array.isArray(containers)) {
    containersEl.innerHTML = `<div class="pill warn"><span>ECS</span><strong>${escapeHtml(containers.error || 'unavailable')}</strong></div>`;
    return;
  }
