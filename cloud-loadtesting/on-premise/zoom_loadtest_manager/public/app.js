...
function containerHealth(container) {
  if (container.pending) {
    return {
      className: 'starting',
      label: 'starting',
      detail: container.startedAt ? `started ${dateLabel(container.startedAt)}` : 'starting'
    };
  }
...
document.addEventListener('click', async event => {
  ...
  try {
    if (action === 'refresh-all') {
      ...
      showPending('Docker login', 'Logging into the configured Docker registry...');
      logAndShow('Docker login', await request('/api/docker/login', { method: 'POST', body: '{}' }));
