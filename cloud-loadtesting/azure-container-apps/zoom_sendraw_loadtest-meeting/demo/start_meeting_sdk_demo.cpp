...
std::string meeting_number, token, meeting_password, recording_token, remote_url, user_name, media_manifest, user_zak;
int jwt_role = 1;
int media_index = -1;
...
void onMeetingJoined() {
	...
	printf("Meeting parameters received.\n");
}
...
bool processJsonValue(const Json& json, const std::string& key, std::string& value) {
	...
	if (it != json.end() && !it->is_null()) {
		...
		if (lower_key.find("token") != std::string::npos ||
			lower_key.find("zak") != std::string::npos ||
...
bool readJsonStringValue(const Json& json, const std::string& key, std::string& value) {
	auto it = json.find(key);
	if (it == json.end() || it->is_null()) {
		return false;
	}
...
bool readResponseStringValue(const Json& json, const std::string& key, std::string& value) {
	if (!json.is_object()) {
		return false;
	}
	if (readJsonStringValue(json, key, value)) {
		return true;
	}

	const char* nested_keys[] = {"data", "result", "payload"};
	for (const char* nested_key : nested_keys) {
		auto nested = json.find(nested_key);
		if (nested != json.end() && nested->is_object() && readJsonStringValue(*nested, key, value)) {
			return true;
		}
	}
	return false;
...
bool readResponseNonEmptyStringValue(const Json& json, const std::string& key, std::string& value) {
	std::string candidate;
	if (!readResponseStringValue(json, key, candidate) || candidate.empty()) {
		return false;
	}
	value = candidate;
	return true;
...
void ReadJsonSettings()
{
	...
	if (!config_json.is_null()) {
		...
		processJsonValue(config_json, "user_zak", user_zak);
		if (user_zak.empty()) {
			processJsonValue(config_json, "zak", user_zak);
		}
...
void StartMeeting()
{
	std::cerr << "Starting Meeting" << std::endl;
	if (user_zak.empty())
	{
		std::cerr << "StartMeeting requires user_zak/ZAK. Set USER_ZAK, ZAK_TOKEN, or return zak/userZAK from the token endpoint." << std::endl;
		std::exit(65);
	}
	...
	{
		std::cerr << "CreateMeetingService:error " << err2 << std::endl;
		std::exit(65);
	}
	...
	ZOOM_SDK_NAMESPACE::StartParam4WithoutLogin& withoutloginParam = startParam.param.withoutloginStart;
	withoutloginParam.userZAK = user_zak.c_str();
	withoutloginParam.userName = user_name.empty() ? "ZoomLoadHost" : user_name.c_str();
	withoutloginParam.zoomuserType = ZOOM_SDK_NAMESPACE::ZoomUserType_APIUSER;
	...
	std::cerr << "ZAK token length is " << user_zak.size() << std::endl;
	std::cerr << "Starting meeting number " << meeting_number << std::endl;
...
std::string getJWTToken(const std::string& remote_url) {
	...
	try {
		...
		{
			std::cerr << "Token endpoint returned invalid JSON." << std::endl;
		}

		if (readResponseNonEmptyStringValue(responses_json, "signature", token))
		{
		}
		else if (readResponseNonEmptyStringValue(responses_json, "token", token))
		{
		}
		else if (readResponseNonEmptyStringValue(responses_json, "jwtToken", token))
		{
		}
		else if (readResponseNonEmptyStringValue(responses_json, "jwt_token", token))
		{
		}

		if (user_zak.empty()) {
			if (readResponseNonEmptyStringValue(responses_json, "zak", user_zak))
			{
			}
			else if (readResponseNonEmptyStringValue(responses_json, "userZAK", user_zak))
			{
			}
			else if (readResponseNonEmptyStringValue(responses_json, "user_zak", user_zak))
			{
			}
			else if (readResponseNonEmptyStringValue(responses_json, "zakToken", user_zak))
			{
			}
			else if (readResponseNonEmptyStringValue(responses_json, "zak_token", user_zak))
			{
			}
		}

		if (useRecordingTokenFromWebService) {
			if (readResponseNonEmptyStringValue(responses_json, "recordingtoken", recording_token))
			{
			}
			else if (readResponseNonEmptyStringValue(responses_json, "recording_token", recording_token))
			{
			}
		}

		std::cerr << "Fetched SDK JWT length: " << token.size() << std::endl;
		std::cerr << "Fetched ZAK length: " << user_zak.size() << std::endl;
	}
