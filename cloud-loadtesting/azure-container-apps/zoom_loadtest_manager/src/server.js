...
import { config, envSnapshot, loadPersistedEnv, publicStatus, saveEnv } from './config.js';
...
import { dockerLogin, killContainers, listContainers, startContainers } from './azure.js';
...
app.post('/api/azure/auth-info', asyncRoute(async (_req, res) => {
  res.json({
    ok: true,
...
app.get('/api/azure/executions', asyncRoute(async (_req, res) => {
