...
MEETING_MODE=${MEETING_MODE:-${LOADTEST_MODE:-${RUN_MODE:-join}}}
MEETING_MODE=$(printf '%s' "$MEETING_MODE" | tr '[:upper:]' '[:lower:]')
case "$MEETING_MODE" in
  join|joinmeeting)
    MEETING_MODE=join
    EXECUTABLE=/opt/zoom-loadtest/bin/ZoomSendRawLoadTestJoinMeetingDemo
    DEFAULT_JWT_ROLE=0
    ;;
  start|startmeeting|host)
    MEETING_MODE=start
    EXECUTABLE=/opt/zoom-loadtest/bin/ZoomSendRawLoadTestStartMeetingDemo
    DEFAULT_JWT_ROLE=1
    ;;
  *)
    echo "MEETING_MODE must be join or start. Got: ${MEETING_MODE}" >&2
    exit 64
    ;;
esac
USER_ZAK=${USER_ZAK:-${ZAK_TOKEN:-${ZOOM_ZAK:-}}}
if [[ "$MEETING_MODE" == "start" && -z "$USER_ZAK" ]]; then
  echo "USER_ZAK or ZAK_TOKEN is required when MEETING_MODE=start." >&2
  exit 64
fi
...
cat > "$CONFIG_FILE" <<JSON
{
  ...
  "jwt_role": "$(json_escape "${JWT_ROLE:-$DEFAULT_JWT_ROLE}")",
...
echo "Starting ${ZOOM_USERNAME} for meeting ${MEETING_NUMBER} in ${MEETING_MODE} mode"
