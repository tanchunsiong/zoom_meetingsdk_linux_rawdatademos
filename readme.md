...
- [BreakoutRoomExample](BreakoutRoomExample/readme.md): monitor and optionally create, assign, start, and stop breakout rooms
...
- [ServiceQualityExample](ServiceQualityExample/readme.md): monitor connection quality and audio, video, and share statistics
...
These samples are developed for Linux environments such as Ubuntu 22, CentOS Stream 9,
and Oracle Linux 8.
...
Dockerfiles are included under the raw-data sample directories. See the
[Docker notes](<readme for docker.md>) for example build and run commands. Docker builds use the
repository root as context and the local `sdk/` directory. Breakout Room and
Service Quality currently use the native CMake workflow only.
